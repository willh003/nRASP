...
Function/DF CreatePackageData() // Called only from GetPackageDFREF
	...
	Variable/G dfr:x_offset = 8
	Variable/G dfr:y_offset = 2
	return dfr
End
...
Function/WAVE getForce()
	...
	NVAR x_offset = dfr:x_offset, y_offset = dfr: y_offset
	...
	reDim(lith_force, y_offset, x_offset)
	return lith_force
end
...
Function reDim(w, rows, cols)
	WAVE w
	Variable rows, cols
	NVAR VSP = root:packages:MFP3D:XPT:Cypher:GlobalVars:'My Globals':VSP

	deletePoints/M=1 0, rows, w
	deletePoints/M=0 0, cols, w
	
	redimension/N=(256,256) w
	Variable i, j
	for (i=256-cols; i < 256; i+=1)
		for (j=0; j < 256; j+=1)
			w[i][j] = VSP
		endfor
	endfor
...
Function NanoRASP_Panel() : Panel
	...
	SetVariable y_offset,pos={458,142},size={120,16},title="Y Tip Offset"
	SetVariable y_offset,help={"y offset due to tip shape"},font="Arial"
	SetVariable y_offset,value= root:packages:MFP3D:XPT:Cypher:GlobalVars:'My Globals':y_offset
	SetVariable x_offset,pos={325,142},size={120,16},title="X Tip Offset", font = "Arial"
	SetVariable x_offset,help={"x offset due to tip shape"},font="Arial"
	SetVariable x_offset,value= root:packages:MFP3D:XPT:Cypher:GlobalVars:'My Globals':x_offset
End